...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  `created_by`      INT UNSIGNED   DEFAULT NULL,
  `updated_by`      INT UNSIGNED   DEFAULT NULL,
  `edited_by`       INT UNSIGNED   DEFAULT NULL COMMENT 'Who last saved an edit via the order edit form — separate from updated_by, which also changes on status/dispatch updates',
...
CREATE TABLE IF NOT EXISTS `order_items` (
  ...
  `returned_qty` INT           NOT NULL DEFAULT 0 COMMENT 'How many of qty have been returned',
...
-- Per-item return log (supports partial returns on multi-item orders)
